        ...
        <div className="flex flex-col md:flex-row gap-8 p-8">
          ...
          <div className="md:w-1/3 flex flex-col items-center text-center">
            ...
            <h2 className="mt-4 text-2xl font-bold text-gray-900">
              {loading ? <span className="animate-pulse bg-gray-200 rounded w-32 h-6 inline-block" /> : userData?.name || "Student Name"}
            </h2>
            <p className="text-gray-600 italic text-sm mt-1">
              {loading ? <span className="animate-pulse bg-gray-100 rounded w-40 h-4 inline-block" /> : "Focused on learning and growing."}
            </p>
            <div className="flex gap-4 mt-4">
              <a href="#" className="text-indigo-600 hover:text-indigo-800"><Linkedin size={20} /></a>
              <a href="#" className="text-gray-700 hover:text-black"><Github size={20} /></a>
            </div>
            {/* Edit Profile Button */}
            <button
              ...
              {openSection === "skills" && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-2">
                  {skills.map((skill, idx) => (
                    <div key={idx} className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2 text-indigo-800 font-medium shadow-sm">
                    ...
                    <div key={idx} className="bg-gray-50 rounded-xl p-4 shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {proj.tech.map((tech, i) => (
                          <span key={i} className="bg-indigo-200 text-indigo-800 px-2 py-0.5 rounded text-xs font-medium">{tech}</span>
                        ))}
                      </div>
                    </div>
                  ))}
